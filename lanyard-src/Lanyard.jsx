...
export default function Lanyard({ position = [0, 0, 22], gravity = [0, -40, 0], fov = 20, transparent = true, cardTexture = null }) {
  return (
    <div className="lanyard-wrapper">
      ...
      >
        ...
        {/* persistent invisible mesh keeps R3F's render loop warm so the Band
            appears as soon as its assets / Rapier finish loading */}
        <mesh visible={false} position={[0, 0, -50]}><boxGeometry /><meshBasicMaterial /></mesh>
...
function Band({ maxSpeed = 50, minSpeed = 0, cardTexture = null }) {
  ...
  return (
    <>
      <group position={[-0.55, 4, 0]}>
        <RigidBody ref={fixed} {...segmentProps} type="fixed" />
        <RigidBody position={[0.35, 0, 0]} ref={j1} {...segmentProps}><BallCollider args={[0.1]} /></RigidBody>
        <RigidBody position={[0.7, 0, 0]} ref={j2} {...segmentProps}><BallCollider args={[0.1]} /></RigidBody>
        <RigidBody position={[1.05, 0, 0]} ref={j3} {...segmentProps}><BallCollider args={[0.1]} /></RigidBody>
        <RigidBody position={[1.35, 0, 0]} ref={card} {...segmentProps} type={dragged ? 'kinematicPosition' : 'dynamic'}>
